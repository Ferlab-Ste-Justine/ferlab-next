...
export const extendColumns = (
  ...
  hasColumnsConfig ||
    console.log(
      '  - No Columns config present. Defaulting to first 5 extended fields.'
    );
            ...
            return column.field
              ? {
                  ...
                  [ConfigProperties.ACCESSOR]:
                    column[ConfigProperties.ACCESSOR] ??
                  ...
                  [ConfigProperties.CAN_CHANGE_SHOW]:
                    column[ConfigProperties.CAN_CHANGE_SHOW],
                  ...
                  [ConfigProperties.IS_ARRAY]:
                    extendedObj?.[ConfigProperties.IS_ARRAY] || false,
                  ////////// TODO!!!!!!!!!!
                  ...
                  [ConfigProperties.QUERY]:
                    column[ConfigProperties.QUERY] ??
                    toQuery(column[ConfigProperties.FIELD_NAME]),
                  // should the column be shown by default
                  [ConfigProperties.SHOW]: column[ConfigProperties.SHOW],
                  // self-descriptive, hopefully
                  [ConfigProperties.SORTABLE]:
                    column[ConfigProperties.SORTABLE],
                }
              : null;
          })
          .filter(Boolean)
...
export const extendFacets = (
  ...
  hasAggsConfig ||
    console.log(
      '  - No Aggregations config present. Defaulting to first 5 extended fields.'
    );
  ...
  const aggs = hasAggsConfig
    ? aggsFromConfig
        .map((agg) => {
          const extendedObj = extendedFields?.find(
            (obj) => obj.field === agg.field.replace(/__/g, '.')
          );
          ...
          return agg.field
            ? {
                ...
                [ConfigProperties.IS_ACTIVE]:
                  agg[ConfigProperties.IS_ACTIVE] || false,
        ...
        ?.map((agg) =>
          ...
          ['nested', 'object'].includes(agg[ConfigProperties.DISPLAY_TYPE])
            ...
            : {
                ...
                // @ts-ignore
                [ConfigProperties.FIELD_NAME]: agg[
                  ConfigProperties.FIELD_NAME
                ].replaceAll('.', '__'),
                // defines aggregation type (component used in facets)
                [ConfigProperties.DISPLAY_TYPE]:
...
export const extendFields = (
  ...
  extendedFromFile: I_ExtendedMappingSetFieldInput[]
) =>
