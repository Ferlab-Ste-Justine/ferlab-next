...
const getAllData = async ({
  ...
  //todo: get mapping and extendedMapping (old getEsMapping fn)
  const extendedFields = await getExtendedFields({
    extendedMapping: null,
    mapping: null,
  });
  const { schema, esClient } = context;
  ...
  // @ts-ignore
  const nestedFieldNames = extendedFields
  ...
  runQuery({
    ...
    .then((res) => {
      const data = res.data;
      const maxHits = ALLOW_CUSTOM_MAX_DOWNLOAD_ROWS
          ...
          .search({
            ...
            body: {
              ...
              ...(previousHits
                ? {
                    search_after:
                      previousHits[previousHits.length - 1]?.sort?.map(
                        esToSafeJsInt
                      ),
                  }
                : {}),
