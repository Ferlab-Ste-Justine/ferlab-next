...
export const columnsToHeader = ({
  columns,
  extendedFieldsDict,
  fileType = 'tsv',
}) => {
  const columnHeaders = columns.reduce(
    (output, { accessor, displayName, field, Header }) => {
      ...
      return {
        ...
        [field || accessor]:
          displayName ||
          Header ||
          fieldDefaultExtendedDetails?.displayName ||
          field,
      };
    },
    {}
...
const rowToTSV = ({ row, valueWhenEmpty }) =>
  row.map((r) => r || valueWhenEmpty).join('\t');
...
const dataToExportFormat = async ({
  columns,
  context = {},
  fileType = 'tsv',
  index,
  ...
  //todo: get mapping and extendedMapping (old getEsMapping fn) (from context ?)
  const extendedFields = await getExtendedFields({
    extendedMapping: null,
    mapping: null,
  });
